...
const ChatHistory: React.FC<ChatHistoryProps> = ({}:ChatHistoryProps) => {
  ...
  const  {transcriptText, setTranscript}  = React.useContext<MyChatContextProps>(MyChatContext);
  ...
  function addNewMessageToChatAvailable(): void {
    ...
    chats.forEach((chat) => {
      ...
      if (currentChat.active == false && messagedAdded == false) {
        ...
        chat.setChat({ active: true, positionRef: 0, text: transcriptText, transition: 'bottom 1s ease-in-out' });
        messagedAdded = true;
      }
    });
    if(messagedAdded==false){
      ...
      moveChatsUp();
      maxPositionRefObj.setChat({active:true, positionRef:0, text:transcriptText, transition: 'none'});
    }
  }
